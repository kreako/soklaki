<template>
    <div>
        <div class="flex flex-row">
            ...
            <div class="text-gray-800 flex-grow">Compétences (cycle&nbsp;{{ cycleAsNb }})</div>
            <div class="text-gray-800 flex flex-row items-center space-x-1">
                ...
                >
                    ...
                    <option
                        v-for="subject in socle.subjects"
                        :value="subject.id"
        ...
        <input v-model="socleFilter" class="input w-full mt-2" type="text" placeholder="Filtre..." />
        <div class="mt-2">
            <div v-if="filtered">
                <div v-for="domain in filteredSocle">
                    <div class="flex flex-row space-x-2">
                        <div
                            class="uppercase text-xs tracking-wide text-gray-700"
                        >{{ socle.domains[domain.id].rank }}. {{ socle.domains[domain.id].title }}</div>
                    ...
                    <div class="pl-2" v-for="component in domain.components">
                        <div class="flex flex-row space-x-2">
                            ...
                            >{{ socle.components[component.id].rank }}. {{ socle.components[component.id].title }}</div>
                            <button
                        ...
                        <div v-for="competency in component.competencies">
                            ...
                            >{{ socle.competencies[competency.id].rank }}. {{ socle.competencies[competency.id].text }}</button>
                        </div>
                    </div>
                </div>
                <div v-if="filteredSocle.length === 0">
                    ...
                    <MascotteTip>
                        ...
                        <template v-slot:default>
                            Oups, les filtres sont peut-être trop sévères, je ne trouve rien à afficher.
            ...
            <div v-else>
                <div v-if="selectedDomain == null">
                    <div v-for="domain in socle[cycle]">
                        ...
                        >{{ socle.domains[domain.id].rank }}. {{ socle.domains[domain.id].title }}</button>
                    </div>
                </div>
                <div v-else>
                    ...
                    <div class="flex flex-row space-x-2">
                        <div
                            class="uppercase text-xs tracking-wide text-gray-700"
                        >{{ socle.domains[selectedDomain].rank }}. {{ socle.domains[selectedDomain].title }}</div>
                        <button @click="deselectDomain">
                            <IconBackSpace class="w-4 text-gray-500" />
                        </button>
                    </div>
                    <div v-for="component in componentsFromDomain(selectedDomain)" class="pl-2">
                        <button
                            @click="selectComponent(component.id)"
                            class="text-gray-700 text-left border border-white hover:border-teal-500"
                        >{{ socle.components[component.id].rank }}. {{ socle.components[component.id].title }}</button>
                    </div>
                </div>
            </div>
            <!--
...
const filteredSocle = computed(() => {
            ...
            for (const component of domain.components) {
                ...
                for (const competency of component.competencies) {
                    if (props.socle.competencies[competency.id].rank == competencyRank) {
                        return [{
                            id: domain.id, components: [
                                {
                                    id: component.id, competencies: [
                                        competency
                                    ]
                                }
                            ]
    ...
    for (const domain of props.socle[props.cycle]) {
        ...
        for (const component of domain.components) {
            if ((selectedComponent.value != null) && (component.id !== selectedComponent.value)) {
            ...
            for (const competency of component.competencies) {
                ...
                if (r.test(props.socle.competencies[competency.id].text)) {
                    ...
                    const d = searchObjectIdInArray(socle, { id: domain.id, components: [] })
                    const c = searchObjectIdInArray(d.components, { id: component.id, competencies: [] })
...
const filtered = computed(() => ((selectedSubject.value !== -1) || (socleFilter.value !== '') || ((selectedDomain.value != null) && (selectedComponent.value != null))))
