...
pub async fn sorted(db: db::Db, token: jwt::JwtToken) -> Json<SortedCompetencies> {
    ...
    let c1 = db
        .run(move |client| {
            sorted_competencies_by_cycle(client, &group_id.clone(), &cycle::Cycle::C1)
        })
        .await
        .unwrap();
    let c2 = db
        .run(move |client| {
            sorted_competencies_by_cycle(client, &group_id.clone(), &cycle::Cycle::C2)
        })
        .await
        .unwrap();
    let c3 = db
        .run(move |client| {
            sorted_competencies_by_cycle(client, &group_id.clone(), &cycle::Cycle::C3)
        })
        .await
        .unwrap();
    let c4 = db
        .run(move |client| {
            sorted_competencies_by_cycle(client, &group_id.clone(), &cycle::Cycle::C4)
        })
        .await
        .unwrap();
